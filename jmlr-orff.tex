...
understanding of \acp{ORFF}. First we extend the construction methodology of
\acs{ORFF} (from an \acs{OVK}), to learn functions whose domain is any arbitrary \acl{LCA} groups and
...
to construct an \acs{ORFF} ex-nihilo such that it yields a valid \acs{OVK}. We
...
\acsp{OVK}.  Then we present two high probability error bounds on the (stochastic)
...
work \citep{brault2016random} to any Locally compact abelian group. Moreover we
...
Eventually we show how to deploy a simple learning algorithm for kernel Ridge
...
conclude with a numerical analysis on toy and real datasets. In particular we
tried our method on a new dataset SARCOS which was not present in
\citet{brault2016random}.
...
equation. The theory of \acsp{OVK} which extends the scalar-valued kernel was
...
\paragraph{}
We present a construction of feature maps called \acs{ORFF}, such that $f:
